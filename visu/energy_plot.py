...
for item in os.listdir(events):
        ...
        if m.group(1) == 'centralized' and int(m.group(3)) == n_hosts:
            ...
            with open(event_file, 'r') as f:
                n_vms[turn_off][algo] = {}
                violation_time[turn_off][algo] = 0
                n_violations[turn_off][algo] = 0
                ...
                for line in f:
                    try:
                        ...
                        if event['value'] == "violation-out":
                            last_violation[event['origin']] = float(event['time'])
...
########################################
# Make the violations plot
########################################
ind = np.arange(len(ORDER)) # the x locations for the groups
width = 0.35

ordered_violation_time = {True: [], False: []}
ordered_n_violations = {True: [], False: []}
for alg in ORDER:
    ordered_violation_time[False].append(energy[False][alg])
    ordered_n_violations[True].append(energy[True][alg])
...
color2 = '#FFFFFF'
linewidth = 1
rects1 = ax1.bar(ind, ordered_violation_time[False], width, color=color1, linewidth=linewidth)
rects2 = ax1.bar(ind + width, ordered_violation_time[True], width, color=color2, linewidth=linewidth)

ax1.set_ylabel('Violation time (seconds)')
ax1.set_xticks(ind + width)
lim = ax1.get_ylim()
ax1.set_ylim(lim[0], lim[1])
...
save_path = find_filename('violations_%d_%d_%%d.pdf' % (load, std))
plt.savefig(save_path, transparent=True, bbox_extra_artists=(lgd,), bbox_inches='tight')
print('Saved plot as ' + save_path)
if os.system('which imgcat > /dev/null 2>&1') == 0:
    os.system('imgcat ' + save_path)
