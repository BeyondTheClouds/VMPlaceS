...
public abstract class AbstractScheduler implements Scheduler {
     ...
     */
	public void relocateVM2(final String vmName, final String sourceName, final String destName) {

        incOngoingMigrations();
        // Who added that ?
        currentMigrations.add(SimulatorManager.getXVMByName(vmName));

        if(!SimulatorManager.migrateVM(vmName, sourceName, destName))
            rpAborted =true;

        // Who added that ?
        decOngoingMigrations();
        currentMigrations.remove(SimulatorManager.getXVMByName(vmName));
